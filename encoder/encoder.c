...
static void frame_dump( x264_t *h )
{
    ...
    /* Write the picture in display order */
    ...
    {
        if( PARAM_FIELD_ENCODE && h->sh.b_bottom_field && fseek( f, (int64_t)h->param.i_width * sizeof(pixel), SEEK_CUR ) )
            goto end;
        ...
        for( int p = 0; p < (CHROMA444 ? 3 : 1); p++ )
            for( int y = 0; y < h->param.i_height; y++ )
            {
                ...
                if( PARAM_FIELD_ENCODE && fseek( f, (int64_t)h->param.i_width * sizeof(pixel), SEEK_CUR ) )
                    goto end;
            }
        ...
        if( !CHROMA444 )
        {
            ...
            if( planeu )
            {
                ...
                {
                    if( h->sh.b_bottom_field && fseek( f, -(int64_t)(h->param.i_width>>1) * sizeof(pixel), SEEK_CUR ) )
                        goto end;

                    for( int y = 0; y < ch; y++ )
                    {
                        fwrite( &planeu[y*cw], sizeof(pixel), cw, f );
                        if( fseek( f, (int64_t)cw * sizeof(pixel), SEEK_CUR ) )
                            goto end;
                    }
                    for( int y = 0; y < ch; y++ )
                    {
